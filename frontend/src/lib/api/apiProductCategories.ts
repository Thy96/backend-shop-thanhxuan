...
export async function getProductCategories() {
  const res = await fetch(`${API_URL}/api/admin/products/categories`, { cache: "no-store", credentials: 'include', });
  return res.json();
}

export async function getProductCategoryById(id: string) {
  const res = await fetch(`${API_URL}/api/admin/products/categories/${id}`, { cache: "no-store" });
  return res.json();
}

export async function createProductCategory(categories: { name: string }) {
...
export async function updateProductCategory(id: string, categories: { name: string }) {
  const formData = new FormData();
...
export async function deleteProductCategory(id: string, cookieHeader?: string) {
  const res = await fetch(`${API_URL}/api/admin/products/categories/${id}`, {
    method: 'DELETE',
